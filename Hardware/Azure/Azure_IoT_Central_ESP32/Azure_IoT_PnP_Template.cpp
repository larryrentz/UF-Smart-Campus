...
#define TELEMETRY_PROP_NAME_LED1                       "LED1"
#define TELEMETRY_PROP_NAME_LED2                       "LED2"
#define TELEMETRY_PROP_NAME_LED3                       "LED3"
#define TELEMETRY_PROP_NAME_LED4                       "LED4"
...
static az_span COMMAND_NAME_TOGGLE_LED_3 = AZ_SPAN_FROM_STR("ToggleLed3");
static az_span COMMAND_NAME_TOGGLE_LED_4 = AZ_SPAN_FROM_STR("ToggleLed4");
...
/* Analog values for the voltage for the LEDs */
float LED1; 
float LED2; 
float LED3; 
float LED4;
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_LED_1))
  {
    ...
    //toggle mosfet state and log
    LED1_MOSFET_STATE = !LED1_MOSFET_STATE;
    LogInfo("LED 1 Mosfet state: %s", (LED1_MOSFET_STATE ? "OPEN" : "CLOSED"));
  ...
  {
    LED2_MOSFET_STATE = !LED2_MOSFET_STATE;
    LogInfo("LED 2 Mosfet state: %s", (LED2_MOSFET_STATE ? "OPEN" : "CLOSED"));
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_LED_3))
  {
    LED3_MOSFET_STATE = !LED3_MOSFET_STATE;
    LogInfo("LED 3 Mosfet state: %s", (LED3_MOSFET_STATE ? "OPEN" : "CLOSED"));
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_LED_4))
  {
    LED4_MOSFET_STATE = !LED4_MOSFET_STATE;
    LogInfo("LED 4 Mosfet state: %s", (LED4_MOSFET_STATE ? "OPEN" : "CLOSED"));
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
...
/* --- Internal Functions --- */
static void get_led_status(int* LED1_STATUS, int* LED2_STATUS, int* LED3_STATUS, int* LED4_STATUS) 
{
  /*
  LED1 = (Vref/4095)*LED1;
  LED2 = (Vref/4095)*LED2;
...
static int simulated_get_temperature()
{
  if (count == 0){
  return 1;
  }
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length)
{
  ...
  int LED1_STATUS, LED2_STATUS, LED3_STATUS, LED4_STATUS;

  float humidity, light, pressure, altitude;
  int LED1_test;
  ...
  LED1_test = simulated_get_temperature();
  ...
  get_led_status(&LED1_STATUS, &LED2_STATUS, &LED3_STATUS, &LED4_STATUS);
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_LED1));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, LED1_STATUS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property value to telemetry payload.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_LED2));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, LED2_STATUS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property value to telemetry payload.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_LED3));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, LED3_STATUS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property value to telemetry payload.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_LED4));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, LED4_STATUS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding magnetometer(Y) property value to telemetry payload.");
  ...
  rc = az_json_writer_append_double(&jw, LED1_test, DOUBLE_DECIMAL_PLACE_DIGITS);
